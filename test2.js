const THREE = new Proxy({}, { get: () => new Proxy({}, { get: () => () => ({ set:()=>{}, add:()=>{}, copy:()=>{}, updateMatrix:()=>{} }) }) }); const window = { innerWidth: 1000, innerHeight: 1000, devicePixelRatio: 1, addEventListener: () => {}, AudioContext: class {} }; const document = { body: { appendChild: () => {} }, getElementById: () => ({ style: {}, addEventListener: () => {}, classList: { add:()=>{}, remove:()=>{} } }), createElement: () => ({ getContext: () => ({ fillRect:()=>{}, strokeRect:()=>{} }), style: {} }), querySelector: () => ({ style: {} }) }; 
        ...
        // Tarea 20: Personaje Detallado Estilo Steampunk
        function createDetailedBrawler(colorHex) {
            ...
            // Materiales
            const skinMat = new THREE.MeshStandardMaterial({ color: 0xffdbac }); // Tono piel
            const hairMat = new THREE.MeshStandardMaterial({ color: 0xff4500, roughness: 0.8 }); // Rojo fuego
            const jacketMat = new THREE.MeshStandardMaterial({ color: 0xd2691e }); // Naranja/Marrón cuero
            const shirtMat = new THREE.MeshStandardMaterial({ color: 0x444444 }); // Gris oscuro
            const pantMat = new THREE.MeshStandardMaterial({ color: 0x222222 }); // Negro
            const detailMat = new THREE.MeshStandardMaterial({ color: 0xffcc00, metalness: 0.8, roughness: 0.2 }); // Oro/Latón
            const bootMat = new THREE.MeshStandardMaterial({ color: 0x3d2b1f }); // Botas marrones

            // 1. Cuerpo (Tronco)
            const bodyGeo = new THREE.CylinderGeometry(0.5, 0.4, 0.8, 8);
            const body = new THREE.Mesh(bodyGeo, jacketMat);
            body.position.y = 1.0;
            visualGroup.add(body);

            // Camisa debajo
            const shirtGeo = new THREE.CylinderGeometry(0.35, 0.35, 0.2, 8);
            const shirt = new THREE.Mesh(shirtGeo, shirtMat);
            shirt.position.y = 1.3;
            visualGroup.add(shirt);

            // 2. Cabeza
            const headGeo = new THREE.BoxGeometry(0.6, 0.6, 0.55);
            const head = new THREE.Mesh(headGeo, skinMat);
            head.position.y = 1.65;
            visualGroup.add(head);

            // Bandana amarilla
            const bandanaGeo = new THREE.BoxGeometry(0.65, 0.15, 0.6);
            const bandana = new THREE.Mesh(bandanaGeo, detailMat);
            bandana.position.y = 1.8;
            visualGroup.add(bandana);

            // Pelo Rojo (Erizado)
            const hairGroup = new THREE.Group();
            for(let i=0; i<12; i++) {
                const hGeo = new THREE.ConeGeometry(0.15, 0.4, 4);
                ...
                h.rotation.x = (Math.random()-0.5)*1;
                h.rotation.z = (Math.random()-0.5)*1;
                hairGroup.add(h);
            }
            visualGroup.add(hairGroup);

            // 3. Piernas
            const legGeo = new THREE.CylinderGeometry(0.15, 0.12, 0.6, 8);
            ...
            // Botas
            const bootGeo = new THREE.BoxGeometry(0.25, 0.2, 0.4);
            const leftBoot = new THREE.Mesh(bootGeo, bootMat);
            leftBoot.position.set(-0.25, 0.1, 0.1);
            visualGroup.add(leftBoot);

            const rightBoot = new THREE.Mesh(bootGeo, bootMat);
            rightBoot.position.set(0.25, 0.1, 0.1);
            ...
            visualGroup.add(leftArm);
            group.userData.leftArm = leftArm;

            const rightArm = new THREE.Mesh(armGeo, jacketMat);
            rightArm.position.set(0.65, 1.1, 0);
            rightArm.rotation.z = -0.2;
            visualGroup.add(rightArm);
            group.userData.rightArm = rightArm;

            // 5. Armas (Pistolas Steampunk x2)
            function createGun() {
                const gunGroup = new THREE.Group();
                // Cañón
                const bGeo = new THREE.CylinderGeometry(0.08, 0.08, 0.5, 8);
                bGeo.rotateX(Math.PI/2);
                const b = new THREE.Mesh(bGeo, detailMat);
                gunGroup.add(b);
                // Cuerpo/Culara
                const cGeo = new THREE.BoxGeometry(0.15, 0.25, 0.3);
                const c = new THREE.Mesh(cGeo, pantMat);
                c.position.z = -0.2;
                gunGroup.add(c);
                // Luz emissiva azul
                const glowGeo = new THREE.SphereGeometry(0.05, 8, 8);
                const glowMat = new THREE.MeshStandardMaterial({ color: 0x00E5FF, emissive: 0x00E5FF, emissiveIntensity: 2 });
            ...
            leftGun.position.set(-0.8, 1.0, 0.3);
            visualGroup.add(leftGun);
            group.userData.leftGun = leftGun;

            const rightGun = createGun();
            rightGun.position.set(0.8, 1.0, 0.3);
            visualGroup.add(rightGun);
            group.userData.rightGun = rightGun;

            // Guardar material para flash (usaremos el de la chaqueta por simplicidad)
            group.userData.bodyMaterial = jacketMat;

            // Sombra bajo el personaje (plano sutil)
        ...
        function updateCharacterAnimation(group, state, time, delta) {
            const { leftLeg, rightLeg, leftArm, rightArm, leftGun, rightGun } = group.userData;
            if(!leftLeg) return;
            ...
            switch(state) {
                ...
                case 'walk':
                    ...
                    rightArm.rotation.x = legRot * 0.8;
                    group.getObjectByName("visualGroup").position.y = Math.abs(Math.sin(time * walkSpeed)) * 0.1;
                    break;
                case 'attack':
                    // Recoil rápido
                    const recoil = Math.sin(time * 20) * 0.2;
                    leftGun.position.z = 0.3 - recoil;
                    rightGun.position.z = 0.3 - recoil;
                    break;
            }
        }
        ...
        function initMenu3D() {
            ...
            function animateMenu() {
                if (startMenu.style.display !== 'none') {
                    ...
                    updateCharacterAnimation(menuBrawler, 'idle', now, 0.016);
                    menuBrawler.rotation.y += 0.005;
        ...
        function animate() {
            ...
            if (isPlaying) {
                ...
                for (let i = enemies.length - 1; i >= 0; i--) {
                    ...
                    // Tarea 9.4: Revelación bots por proximidad al jugador corregido
                    ...
                    enemy.mesh.children.forEach(c => {
                        ...
                        // Tarea 9.4: Bots invisibles (opacidad 0) si están ocultos
                        c.material.opacity = enemy.isVisible ? 1.0 : 0.0;
                        c.material.depthWrite = enemy.isVisible;
                    });

                    // Actualizar animaciones (Tarea 20)
                    ...
                    enemies.forEach(enemy => {
                        let eState = 'idle';
                        if (enemy.target || enemy.cubeTarget) eState = 'walk';
                        if (Date.now() - enemy.lastShot < 200) eState = 'attack';
                        updateCharacterAnimation(enemy.mesh, eState, animNow, delta);
                    });
                    ...
                    for (let j = bullets.length - 1; j >= 0; j--) {
                        ...
                        if (distToEnemy < 1.2) {
                            ...
                            if (enemy.hp <= 0) {
                                ...
                                enemy.dead = true; // Flag para evitar procesar muertes dobles
